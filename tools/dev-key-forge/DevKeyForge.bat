...
Write-Host ('  Valid Thru: ' + $cert.NotAfter);
Write-Host '  Stored in : CurrentUser\Personal Certificates';
" 2>nul
echo.
pause
goto :MAIN_MENU
...
echo  ----------------------------------------------------------------------
powershell -NoProfile -Command "
function Get-CryptString($len) {
    $chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_#$@!';
    $bytes = New-Object byte[] $len;
    (New-Object Security.Cryptography.RNGCryptoServiceProvider).GetBytes($bytes);
    $out = '';
    for ($i = 0; $i -lt $len; $i++) { $out += $chars[$bytes[$i] % $chars.Length]; }
    return $out;
}
Write-Host '  32-Character Key  :' -ForegroundColor Cyan;
Write-Host ('  ' + (Get-CryptString 32)) -ForegroundColor White;
Write-Host '';
Write-Host '  64-Character Key  :' -ForegroundColor Cyan;
Write-Host ('  ' + (Get-CryptString 64)) -ForegroundColor White;
Write-Host '';
Write-Host '  128-Character Key :' -ForegroundColor Cyan;
Write-Host ('  ' + (Get-CryptString 128)) -ForegroundColor White;
" 2>nul
echo.
pause
goto :MAIN_MENU
...
Write-Host '  Base64 Encoded (512-bit):' -ForegroundColor Cyan;
Write-Host ('  ' + $b64) -ForegroundColor Green;
Write-Host '';
Write-Host '  Hex Encoded (512-bit):' -ForegroundColor Cyan;
...
if not defined H_FILE goto :MAIN_MENU
set "H_FILE=%H_FILE:"=%"
...
powershell -NoProfile -Command "
$p = '%H_FILE%';
Write-Host ('  MD5    : ' + (Get-FileHash -Path $p -Algorithm MD5).Hash);
Write-Host ('  SHA1   : ' + (Get-FileHash -Path $p -Algorithm SHA1).Hash);
Write-Host ('  SHA256 : ' + (Get-FileHash -Path $p -Algorithm SHA256).Hash) -ForegroundColor Green;
Write-Host ('  SHA512 : ' + (Get-FileHash -Path $p -Algorithm SHA512).Hash) -ForegroundColor Cyan;
" 2>nul
echo.
pause
goto :MAIN_MENU
...
if "%B_ACT%"=="1" (
    ...
    if defined B_TXT (
        powershell -NoProfile -Command "
        $b = [Text.Encoding]::UTF8.GetBytes('%B_TXT%');
        Write-Host ('  Base64: ' + [Convert]::ToBase64String($b)) -ForegroundColor Green;
        "
    )
)
if "%B_ACT%"=="2" (
    ...
    if defined B_RAW (
        powershell -NoProfile -Command "
        try {
            $bytes = [Convert]::FromBase64String('%B_RAW%');
            Write-Host ('  Decoded: ' + [Text.Encoding]::UTF8.GetString($bytes)) -ForegroundColor Green;
        } catch {
...
powershell -NoProfile -Command "
for ($i=1; $i -le 5; $i++) {
    Write-Host ('  ' + [Guid]::NewGuid().ToString()) -ForegroundColor Green;
}
" 2>nul
echo.
pause
